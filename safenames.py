...
def has_warning(item, root):
    ...
    if '/Photos Library.photoslibrary' in root:
        messages.append('WARNING: "{}" is part of the MacOS Photos App Library. Will not change.'.format(item))
        return True, messages

    if ':' in item and (item.endswith('.abcdg') or item.endswith('.abcdi') or item.endswith('.abcdp') or item.endswith('.abcds')):
        messages.append(
                'WARNING: "{}" contains an illegal colon but MacOS Address Book file. Will not change.'.format(item))
        return True, messages

    if '.AppleDouble' in root:
        if '.Parent::EA' in item or '::EA::' in item or '::EA' in item:
            messages.append(
                    'WARNING: "{}" contains an illegal colon but is an MacOS Appledouble file. Will not change.'.format(item))
            return True, messages
...
#         return
    ...
    if item_clean != item:
        ...
        if not warn_bool:
            ...
            print('Replace "{}" with "{}"?  (Y/n/t/x : Yes/no/type/delete): y'.format(item,
                                                                                      item_clean), end='', flush=True)
...
def main():
    ...
    parser.add_argument("input", nargs='*',
                        default=None, help="accepts files and/or folders to be cleaned")
        ...
        if os.path.isdir(absinput):
            for root, dirs, files in os.walk(absinput, topdown=False):
                ...
                for item in all_items:
                    ...
                    item_cleaned = clean_item(item, root)
                    debug('item="{}", item_cleaned="{}"'.format(
                        item, item_cleaned))
